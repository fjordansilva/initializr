...
public final class InitializrMetadataBuilder {
     ...
     *
     * @param properties
     *         the settings to merge onto this instance
     * @param mergeConfiguration
     *         specify if service configuration should be merged as well
     *
     * @return this instance
     */
    public InitializrMetadataBuilder withInitializrProperties(
     ...
     *
     * @param resource
     *         a resource to a json document describing the metadata to include
     *
     * @return this instance
     */
    ...
    protected void applyDefaults(InitializrMetadata metadata) {
        if (!StringUtils.hasText(metadata.getIrn().getContent())) {
            metadata.getIrn().setContent("IRN-XXXXX");
        }
        if (!StringUtils.hasText(metadata.getSia().getContent())) {
            metadata.getSia().setContent("XXX");
        }
