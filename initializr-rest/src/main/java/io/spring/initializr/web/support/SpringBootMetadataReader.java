...
class SpringBootMetadataReader {
    ...
    SpringBootMetadataReader(ObjectMapper objectMapper, RestTemplate restTemplate, String url) throws IOException {
        content = objectMapper.readTree(restTemplate.getForObject(url, String.class));
    }
     ...
     * @return the versions
     */
    public List<DefaultMetadataElement> getBootVersions(List<String> springBootTags) {
        ...
        for (JsonNode node : releases) {
            // FJOR: condicion para que solo aparezcan las versiones estables y no SNAPSHOTS o intermedias (Mx)
            String versionTag = node.get("releaseStatus").textValue();
            if (springBootTags.contains(versionTag)) {
