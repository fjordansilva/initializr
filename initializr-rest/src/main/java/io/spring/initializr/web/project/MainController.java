...
import io.spring.initializr.web.mapper.*;
...
public class MainController extends AbstractInitializrController {
    ...
    public MainController(InitializrMetadataProvider metadataProvider,
        ...
        commandLineHelpGenerator = new CommandLineHelpGenerator(templateRenderer);
    ...
    public ResponseEntity<String> serviceCapabilitiesHal() {
        return serviceCapabilitiesFor(InitializrMetadataVersion.V2_1,
                MainController.HAL_JSON_CONTENT_TYPE);
    }
    ...
    private ResponseEntity<String> serviceCapabilitiesFor(
        ...
        String appUrl  = generateAppUrl();
        String content = MainController.getJsonMapper(version).write(metadataProvider.get(), appUrl);
    ...
    private ResponseEntity<String> dependenciesFor(InitializrMetadataVersion version, String bootVersion) {
        ...
        DependencyMetadata dependencyMetadata = dependencyMetadataProvider.get(metadata, v);
    ...
    public ResponseEntity<byte[]> pom(ProjectRequest request) {
        ...
        byte[] mavenPom = projectGenerationInvoker.invokeBuildGeneration(request);
    ...
    public ResponseEntity<byte[]> gradle(ProjectRequest request) {
        ...
        byte[] gradleBuild = projectGenerationInvoker.invokeBuildGeneration(request);
    ...
    public ResponseEntity<byte[]> springZip(ProjectRequest request) throws IOException {
        ...
        File                    download      = projectGenerationInvoker.createDistributionFile(dir, ".zip");
        String                  wrapperScript = MainController.getWrapperScript(result.getProjectDescription());
        Zip                     zip           = new Zip();
    ...
    public ResponseEntity<byte[]> springTgz(ProjectRequest request) throws IOException {
        ProjectGenerationResult result = projectGenerationInvoker
                .invokeProjectStructureGeneration(request);
        File dir = result.getRootDirectory().toFile();
        File download = projectGenerationInvoker.createDistributionFile(dir,
                ".tar.gz");
        String wrapperScript = MainController.getWrapperScript(result.getProjectDescription());
        Tar    zip           = new Tar();
        zip.setProject(new Project());
    ...
    private String generateFileName(ProjectRequest request, String extension) {
        String candidate = (StringUtils.hasText(request.getArtifactId())
                ...
                : metadataProvider.get().getArtifactId().getContent());
    ...
    private ResponseEntity<byte[]> upload(File download, File dir, String fileName,
        ...
        MainController.logger.info(String.format("Uploading: %s (%s bytes)", download, bytes.length));
