...
public class DefaultInitializrMetadataUpdateStrategy implements InitializrMetadataUpdateStrategy {
    ...
    /**
     * List of valid Spring Boot version Tags
     */
    private List<String> springBootTags;
    ...
    public InitializrMetadata update(InitializrMetadata current) {
        ...
        springBootTags = current.getConfiguration().getEnv().getSpringBootVersionTags();
     ...
     *
     * @param url
     *         the url to the spring-boot project metadata
     *
    ...
    protected List<DefaultMetadataElement> fetchSpringBootVersions(String url) {
        if (StringUtils.hasText(url)) {
            try {
                DefaultInitializrMetadataUpdateStrategy.logger.info("Fetching Spring Boot metadata from " + url);
                return new SpringBootMetadataReader(objectMapper, restTemplate, url).getBootVersions(springBootTags);
            } catch (Exception ex) {
                DefaultInitializrMetadataUpdateStrategy.logger.warn("Failed to fetch Spring Boot metadata", ex);
            }
        }
        return null;
    }
