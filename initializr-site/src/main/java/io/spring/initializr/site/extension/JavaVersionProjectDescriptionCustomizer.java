...
public class JavaVersionProjectDescriptionCustomizer
    ...
    public void customize(ProjectDescription description) {
        ...
        if (isSpringBootVersionBefore(description, JavaVersionProjectDescriptionCustomizer.VERSION_2_0_0_M1)) {
			updateTo(description, "1.8");
        }
        // Not supported for Kotlin & Groovy
        if (JavaVersionProjectDescriptionCustomizer.UNSUPPORTED_LANGUAGES.contains(description.getLanguage().id())) {
			updateTo(description, "1.8");
        }
        // 10 support only as of 2.0.1
        if (javaGeneration == 10
                && isSpringBootVersionBefore(description, JavaVersionProjectDescriptionCustomizer.VERSION_2_0_1)) {
			updateTo(description, "1.8");
        }
        // 11 and 12 support only as of 2.1.x
        if ((javaGeneration == 11 || javaGeneration == 12)
                && isSpringBootVersionBefore(description, JavaVersionProjectDescriptionCustomizer.VERSION_2_1_0_M1)) {
			updateTo(description, "1.8");
        }
    }
