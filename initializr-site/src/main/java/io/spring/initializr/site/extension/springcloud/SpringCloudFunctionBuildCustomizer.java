...
class SpringCloudFunctionBuildCustomizer implements BuildCustomizer<Build> {
    ...
    private void removeCloudFunction(Build build) {
        Dependency cloudFunction = metadata.getDependencies().get("cloud-function");
    ...
    private BillOfMaterials resolveBom(String id) {
        BillOfMaterials bom = metadata.getConfiguration().getEnv().getBoms().get(id);
        if (bom != null) {
            return bom.resolve(description.getPlatformVersion());
        }
        return null;
    }

    private boolean isSpringBootVersionAtLeastAfter() {
        return (SpringCloudFunctionBuildCustomizer.VERSION_2_1_0_M1.compareTo(description.getPlatformVersion()) <= 0);
    }
